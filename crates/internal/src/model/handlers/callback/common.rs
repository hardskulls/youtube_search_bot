...
pub(crate) async fn get_required_callback_data(
...
pub(crate) async fn handle_callback(
    ...
    let res = match &buttons {
        SearchButtons(search_kb) => {
            callback_helper_for_search_kb(search_kb, d_data, callback).await
        }
        ...
        SearchVideoInPlaylistsButtons(search_video_oin_pls_kb) => {
            callback_helper_for_search_videos_in_playlists_kb(
                search_video_oin_pls_kb,
                d_data,
                callback,
            )
            .await
...
pub(crate) async fn update_and_return_access_token(
    user_id: &str,
    ...
    refresh_access_token(user_id, token, db_url, token_req)
        .await?
        .access_token
        .in_ok()
}
