    ...
    T: YouTubeApiRequestBuilder,
{
    let resp = req_builder
        .build_req(client, access_token, page_token)?
        .send()
        .await?;
...
{
    let keys = (
        RequiredAuthURLParams::ClientId,
        RequiredAuthURLParams::RedirectUri,
        RequiredAuthURLParams::ResponseType,
    );
    let required_params = [
        (keys.0.to_string(), client_id),
        (keys.1.to_string(), redirect_uri),
        (keys.2.to_string(), response_type),
    ];
    ...
    fn test_build_req_with_problematic_pair_test() {
        ...
        impl<'a> YouTubeApiRequestBuilder for TestRequester<'a> {
            ...
            fn build_req(
                ...
                let mut req = client
                    .get(reqwest::Url::parse(YOUTUBE_PLAYLIST_ITEMS_API)?)
                    .query(&[
        ...
        let (access_token, req_builder, page_token) = (
            "gfTHy^75$367Frt%4dHHJytE$#A",
            TestRequester { playlist_id },
            None,
        );
        let _build = req_builder
            .build_req(&client, access_token, page_token)
