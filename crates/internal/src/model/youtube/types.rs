...
mod tests {
    ...
    fn serialize_deserialize_string_test() {
        ...
        let (scope, token_type) = (vec!["hey".to_owned()], "id_token".to_owned());
    ...
    fn deserialize_from_json_test() {
        ...
        assert!(
            matches!(deserialized_token, Ok(_)),
            "cause: {deserialized_token:?}"
        );

        let deserialized_token = dbg!(deserialized_token.unwrap());

        assert!(!deserialized_token.scope.is_empty());
        assert!(deserialized_token
            .scope
            .contains(&"https://www.googleapis.com/auth/drive.metadata.readonly".to_owned()));
        assert!(
            deserialized_token.expires_in > time::OffsetDateTime::now_utc() + Duration::minutes(59)
        );
        assert_eq!(deserialized_token.access_token, "1/fFAGRNJru1FTz70BzhT3Zg");
        assert!(matches!(deserialized_token.refresh_token, Some(_)));
        assert_eq!(
            deserialized_token.refresh_token.as_ref().unwrap(),
            "1//xEoDL4iW3cxlI7yDbSRFYNG01kVKM2C-259HOF2aQbI"
        );

        let serialized_token = dbg!(serde_json::to_string(&deserialized_token).unwrap());
        let _deserialized_again_token =
            dbg!(serde_json::from_str::<YouTubeAccessToken>(&serialized_token).unwrap());
    }

    #[test]
    fn subscription_list_resp_deserialize_test() {
        let subs =
            std::fs::read_to_string("../../test_assets/subscription_list_json_response.json")
                .unwrap();
        ...
        assert!(matches!(
            s.snippet.as_ref().unwrap().resource_id.as_ref(),
            Some(..)
        ));
    }
}
