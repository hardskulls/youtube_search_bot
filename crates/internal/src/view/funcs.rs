...
use crate::model::dialogue::types::{CommandConfig, DialogueData, MessageWithKB, TheDialogue};
...
async fn send_keyboard(
    ...
    match (res, opt_dialogue_data) {
        (Ok(sent_msg), Some(d_data)) if pin_msg => {
            let message_with_kb = MessageWithKB {
                opt_message: sent_msg.into(),
            };
            let new_dialogue_data = DialogueData {
                message_with_kb,
                ..d_data
            };
...
async fn send_results(
    ...
    let formatting = |i: SearchableItem| {
